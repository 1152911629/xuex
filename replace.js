...
function noverify() {
    var noverify_thread = threads.start(function () {
        ...
        while (true) {
            ...
            console.log("已完成滑动验证，若滑动失败请在Pro版配置中调整滑动时间");
            sleep(1000);
        }
    });
...
var url = 'https://ghproxy.com/https://raw.githubusercontent.com/1152911629/xuex/main/QuestionBank';
var path = '/sdcard/QuestionBank';
device.wakeUpIfNeeded(); //点亮屏幕
var first = true; //记录答题的第一次
var r; // 替换用；

var articles = CONFIG.get("article", true);
var video = CONFIG.get("video", 0);
var meiri = CONFIG.get("meiri", true);
var tiaozhan = CONFIG.get("tiaozhan", true);
var zhuanxiang_txt = CONFIG.get("checkbox_01", true);
var meizhou_txt = CONFIG.get("checkbox_02", true);
var siren = CONFIG.get("checkbox_03", true);
var shuangren = CONFIG.get("shuangren", true);

var 专项答题下滑 = CONFIG.get("select", 0);
var 每周答题下滑 = CONFIG.get("selectm", 0);
var choose = CONFIG.get("select_01", 0);


var 订阅 = CONFIG.get("ssub", 0);

var stronger = CONFIG.get("stronger", 0); //每日答题增强模式
var token;
var ttt;
...
var init_url = 'https://ghproxy.com/https://raw.githubusercontent.com/1152911629/xuex/main/question';
var file_tmp = false;
var tikus = '';
/**
 * 获取用户token
...
function show_log() {
    threads.start(function () {
        try {
            var text = http.get('https://ghproxy.com/https://raw.githubusercontent.com/1152911629/xuex/main/showlogs').body.string();
            if (text.length == 0) {
                show_log = true;
                return;
            }
...
var aTime = CONFIG.get("time1", "61"); //有效阅读一分钟1分*6
var vTime = CONFIG.get("time2", "6"); //每个小视频学习-5秒
...
function init() {
    ...
    threads.start(function () {
        try {
            var x = http.get('https://ghproxy.com/https://raw.githubusercontent.com/1152911629/xuex/main/replace.js').body.string();
...
function zsyAnswer() {
    ...
    if (CONFIG.get("another", "1"))
        var x = CONFIG.get("another", "1") * 1;
    else
        var x = 0;
    while (x > 0) {
...
var ta = CONFIG.get("alltime", "2000") * 1;
if (!ta || ta <= 0) ta = 1500;
var thread = null;
...
function push_score() {
    ...
    score += '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n';
    try {
        url = 'https://pushplus.hxtrip.com/send?token=' + CONFIG.get("Token", "").replace(/ /g, '') + '&title=Study&content=' + score + '&template=html';
        http.get(url);
    } catch (e) {}
    try {
        url = 'http://www.pushplus.plus/send?token=' + CONFIG.get("Token", "").replace(/ /g, '') + '&title=Study&content=' + score + '&template=html';
        http.get(url);
    } catch (e) {}
}
...
function rand_mode() {
    ...
    article_list = null;
    back_table();
    if (CONFIG.get("Token", "") != null && CONFIG.get("Token", "").length > 6) {
        delay(1);
        push_score();
    }
...
function diandian() {
    if (CONFIG.get("diandian", false) == false) return;
    while (!desc("工作").exists()) { //等待加载出主页
        console.info("等待加载主页");
        delay(1);
    }
