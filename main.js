  ...
  ui.layout(
    <vertical h="*" bg="#FFFFFF">
      ...
      <vertical layout_weight="1" >
        ...
        <button id="OpenBtn" h="40" text="二维码：开通/续费卡密" bg="#FF00FF" textColor="#FFFFFF" marginTop="10" />
        <button id="OrderSearchBtn" h="40" text="通过订单号查询已 购买/开通/续费 的卡密" bg="#FF00FF" textColor="#FFFFFF" marginTop="10" />
  ...
  ui.OpenBtn.click(function () {
    ...
    dialogs.select("请选择购买价格", 价格列表)
      .then(i => {
        let 选的价格ID = 瑞科验证SDK.软件初始化结果.软件价格数组[i].价格类型ID;
        瑞科验证SDK.打开遮罩层("请稍后...");
        let 结果 = 瑞科验证SDK.二维码开通续费卡密(选的价格ID, 卡密Value);
        瑞科验证SDK.关掉遮罩层();
        if (结果.错误编码 != 0) {
          alert("购买出错\r\n错误编码：" + 结果.错误编码 + "\r\n错误消息：" + 结果.错误消息);
          return;
...
function 登录成功后跳转的主界面() {
  ui.layout(
    <vertical h="*" bg="#FFFFFF">
      ...
      <horizontal w="*" gravity="center" marginTop="10">
        <text text="卡密剩余点数：" color="#0000FF"></text>
        <text id="surpluspointvalueTxt" text="" color="#000000"></text>
      </horizontal>
      <horizontal w="*" gravity="center" marginTop="30">
        <text ellipsize="middle" text="以下功能，只有登录成功后才能调用：" size="18" color="#000000"></text>
      </horizontal>
      <horizontal w="*" gravity="center" marginTop="10" >
        <button id="bucklePointBtn" text="扣点" color="#0000FF" />
        <button id="getRemoteVarBtn" text="获取远程变量" color="#0000FF" />
      </horizontal>
      <horizontal w="*" gravity="center" marginTop="10" >
        <button id="DetailBtn" text="卡密详情" color="#0000FF" />
        <button id="updRemarkBtn" text="修改卡密备注" color="#0000FF" />
      </horizontal>
    </vertical>
  );
  ...
  ui.surpluspointvalueTxt.setText("" + 瑞科验证SDK.登录结果.剩余点数);

  //扣点按钮事件
  ui.bucklePointBtn.click(function () {
    //注意：
    //1、您的软件是扣点的方式，才能调用此方法
    //2、如果你后台设置了当天扣点“次数的上限”，那么当天“扣点次数”超过之后，就算再调用“扣点”，也不会给您的软件用户扣点了，
    //此场景适用于：您软件用户当天首次登录就扣点，之后当天再登录就不会扣点了。到了明天后，重新开始计算扣点逻辑，之后以此类推
    //3、参数是你需要扣得点数
    ...
    console.log("卡密或账号剩余点数：" + 瑞科验证SDK.扣点结果.卡密或账号剩余点数);

  });

  //获取远程变量按钮事件
  ui.getRemoteVarBtn.click(function () {
    ...
    let 结果 = 瑞科验证SDK.获取远程变量("测试变量名");
    if (结果.错误编码 != 0) {
      console.log(结果.错误消息);
      return;
    }
    console.log("变量值：" + 结果.变量值);

  });

  //卡密详情按钮事件
  ui.DetailBtn.click(function () {
    //当你对每个卡密的备注设置了内容，那么可以通过此接口获取到备注，还有其它一些信息
    ...
    console.log("服务器时间戳：" + 结果.服务器时间戳);
    console.log("到期时间：" + 结果.到期时间);
    console.log("剩余点数：" + 结果.剩余点数);
    console.log("开通的价格类型ID：" + 结果.开通的价格类型ID);
    console.log("开通的价格类名称：" + 结果.开通的价格类名称);
    console.log("备注：" + 结果.备注);
    console.log("是否已开通：" + 结果.是否已开通);
    console.log("是否已激活：" + 结果.是否已激活);
    console.log("机器码：" + 结果.机器码);
  });

  //修改卡密备注按钮事件
  ui.updRemarkBtn.click(function () {
    //可以通过：登录后的结果和卡密详情接口，来获取卡密的备注
    //此功能，可以对每张卡密设置一些私有数据。这个得看您自己的业务需求，有这样的需求，就调用此接口
    //比如，有些作者有这么个需求：需要用账号登录，然后登录完后，又需要可以针对某张卡密进行扣点
    //那么可以这么来解决：您的软件可以设置为“账号登录方式，消耗时间方式"，然后可以对此卡密通过“修改备注”的功能来设置点数和扣点的功能
    //其实还有很多有用的处理方式，具体看您自己的需求了

    let 结果 = 瑞科验证SDK.修改卡密账号备注("这时我修改的备注内容，您可以针对单张卡放一些私有数据");
    if (结果.错误编码 != 0) {
      console.log(结果.错误消息);
      return;
    }
    console.log("修改卡密账号备注成功！");
